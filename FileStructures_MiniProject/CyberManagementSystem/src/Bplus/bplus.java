...
class records{
static String name,adhar,email_id,phone,addr,keyf,bill,dat;
}
class node{
int [] a=new int[8];
int size;
...
static String name,adhar,email_id,addr,phone,bill,dat,count;
...
for(int i=0;i<no;i++)
recs[i]=new records();
BufferedReader bbr = new BufferedReader(new FileReader("G:\\6th  sem\\FS\\CyberManagement\\bpluss.txt"));
for(int i=0;i<no;i++)
{
...
System.out.println("Date: "+recs[i].dat);
System.out.println("Bill: "+recs[i].bill);
...
System.out.println(recs[i].keyf+"\t\t"+recs[i].name+"\t\t"+recs[i].adhar+"\t\t"+recs[i].email_id+"\t\t"+recs[i].phone+"\t\t"+recs[i].dat+"\t\t"+recs[i].bill+"\t\t"+recs[i].addr+"\t\t\t");
}
bbr.close();
break;
